...
public interface IPurchaseOrderAppService
{
    ...
    Task<PurchaseOrderQuickCreateResultAppDto> QuickCreatePurchaseOrderAsync(PurchaseOrderQuickCreateAppDto dto);
    ...
    Task<ReceiveGoodsResultAppDto> ReceiveItemAsync(ReceiveGoodsAppDto dto);
    Task<BulkReceiveGoodsResultAppDto> BulkReceiveItemsAsync(BulkReceiveGoodsAppDto dto);
    ...
    Task<IList<PurchaseOrderItemAllocationForPoItemAppDto>> GetAllocationsForPurchaseOrderItemsAsync(
        IReadOnlyList<(Guid primaryItemId, Guid secondaryItemId)> purchaseOrderItemIds);
