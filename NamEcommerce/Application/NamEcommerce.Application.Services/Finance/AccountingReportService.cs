...
public sealed class AccountingReportService(
    ...
    IInventoryCostingAppService inventoryCostingService) : IAccountingReportService
{
    ...
    public async Task<IncomeStatementDto> GetIncomeStatementAsync(AccountingPeriod period)
    {
        ...
        var allDeliveryNotes = deliveryNotes.DataSource.ToList();
        ...
        var surchargeRevenue = customerLedgerEntries.DataSource
        ...
        var salesReturns = creditNotes.DataSource
            .Where(cn => cn.CreatedOnUtc >= start && cn.CreatedOnUtc <= end
                      && cn.Status != CreditNoteStatus.Cancelled)
            .Sum(cn => (decimal?)cn.Amount) ?? 0;

        var cogs = costLedger.DataSource
            .Where(e => e.MovementType == InventoryCostMovementType.SaleDispatch
                     && deliveredNoteIds.Contains(e.ReferenceId))
            .Sum(e => e.TotalCost) ?? 0;

        var vendorReturnCost = costLedger.DataSource
            .Where(e => e.MovementType == InventoryCostMovementType.VendorReturn
                     && e.OccurredAtUtc >= start && e.OccurredAtUtc <= end)
            .Sum(e => e.TotalCost) ?? 0;
        grossRevenue -= vendorReturnCost;

        var customerReturnCost = costLedger.DataSource
            .Where(e => e.MovementType == InventoryCostMovementType.CustomerReturn
                     && e.OccurredAtUtc >= start && e.OccurredAtUtc <= end)
            .Sum(e => e.TotalCost) ?? 0;

        var sellingExp = expenses.DataSource
            .Where(e => (e.ExpenseType == ExpenseType.Marketing || e.ExpenseType == ExpenseType.ReturnCost || e.ExpenseType == ExpenseType.Sale)
                     && e.IncurredDate >= start && e.IncurredDate <= end)
            .ToList()
            .Sum(e => (decimal?)e.AmountExcludingTax) ?? 0;

        var adminExp = expenses.DataSource
            .Where(e => (e.ExpenseType == ExpenseType.Management || e.ExpenseType == ExpenseType.Payroll 
                || e.ExpenseType == ExpenseType.Rent || e.ExpenseType == ExpenseType.AssetDisposal
                || e.ExpenseType == ExpenseType.Utilities || e.ExpenseType == ExpenseType.General)
                     && e.IncurredDate >= start && e.IncurredDate <= end)
            .ToList()
            .Sum(e => (decimal?)e.AmountExcludingTax) ?? 0;

        var sellingDepreciation = fixedAssets.DataSource
        ...
        var adminDepreciation = fixedAssets.DataSource
            .Where(a => a.CostCenter == FixedAssetCostCenter.Admin && a.Status != FixedAssetStatus.Disposed)
            .AsEnumerable()
            .Sum(a => SumDepreciationInPeriod(a, start, end));

        var setup = await setupService.GetSetupAsync().ConfigureAwait(false);
    ...
    public async Task<CashFlowStatementDto> GetCashFlowStatementAsync(AccountingPeriod period)
    {
        ...
        var inventoryIncrease = (costLedger.DataSource
            .Where(e => (e.MovementType == InventoryCostMovementType.GoodsReceipt
        ...
        var deliveredNoteIds = deliveryNotes.DataSource
            .Where(dn => dn.Status == DeliveryNoteStatus.Delivered
                      && dn.DeliveredOnUtc >= start && dn.DeliveredOnUtc <= end)
            .Select(dn => dn.Id)
            .ToHashSet();
        ...
        var purchaseExpenses = expenses.DataSource
            .Where(a => (a.IncurredDate >= start && a.IncurredDate <= end) && a.ExpenseType == ExpenseType.Purchase)
            .Sum(a => (decimal?)a.Amount) ?? 0;

        var invChange = -(inventoryIncrease + purchaseExpenses - inventoryDecrease);

        var vatEnd = GetVatPayable(end);
        var vatBegin = GetVatPayable(start.AddDays(-1));
        var vatChange = vatEnd - vatBegin;

        var assetPurchases = -(fixedAssets.DataSource
        ...
        var actualClosingCash = await cashBookService.GetTotalCashAsync(end).ConfigureAwait(false);
    ...
    private async Task<BalanceSheetDto> BuildBalanceSheetAtDateAsync(DateTime asOf)
    {
        ...
        var cashOnHand = await cashBookService.GetCashBalanceAsync(asOf).ConfigureAwait(false);
        var bankBalances = await cashBookService.GetBankBalancesAsync(asOf).ConfigureAwait(false);

        var ar = GetNetAccountsReceivable(asOf);

        var inventory = 0m;
        foreach (var stock in inventoryStocks.DataSource.Where(stock => stock.QuantityOnHand > 0).ToList())
        {
            var cost = await inventoryCostingService.GetCurrentProductCostPriceAsync(stock.ProductId).ConfigureAwait(false);
            inventory += cost * stock.QuantityOnHand;
        }

        var purchaseExpenses = expenses.DataSource
            .Where(a => a.IncurredDate <= asOf && a.ExpenseType == ExpenseType.Purchase)
            .Sum(a => (decimal?)a.Amount) ?? 0;
        inventory += purchaseExpenses;
        ...
        var inTransitNoteIds = deliveryNotes.DataSource
        ...
        var openingInventoryCost = openingReceiptIds.Count > 0
            ? (costLedger.DataSource
                ...
                .Sum(e => e.TotalCost) ?? 0)
            : 0m;
        var openingCustomerAR = customerLedgerEntries.DataSource
            .Where(e => e.EntryType == CustomerLedgerEntryType.OpeningBalance)
            .Sum(e => (decimal?)e.Amount) ?? 0;
        var openingVendorAP = vendorLedgerEntries.DataSource
    ...
    private decimal GetOperationalAccountsPayable(DateTime asOf)
    {
        ...
        return vendorLedgerEntries.DataSource
    ...
    {
        var cutoff = asOf.Date.AddDays(1).AddTicks(-1);
        return customerLedgerEntries.DataSource
    ...
    private decimal GetVatPayable(DateTime asOf)
    {
        ...
        var vatOut = deliveryNotes.DataSource
            .Where(dn => deliveredNoteIds.Contains(dn.Id))
            .SelectMany(dn => dn.Items)
            .Sum(i => (decimal?)i.TaxAmount) ?? 0;

        var vatIn = expenses.DataSource
            .Where(e => e.IncurredDate <= cutoff)
            .Sum(e => (decimal?)e.TaxAmount) ?? 0;

        var vatInGoods = goodsReceipts.DataSource
    ...
    private async Task<decimal> GetOpeningCashForPeriodAsync(AccountingPeriod period, AccountingSetupAppDto setup)
    {
        ...
        if (period.Start <= fiscalStart)
            return setup.OpeningCash + (await cashBookService.GetBankBalancesAsync(fiscalStart).ConfigureAwait(false)).Sum(b => b.Balance);
        return await cashBookService.GetTotalCashAsync(period.Start.AddDays(-1)).ConfigureAwait(false);
    }
}
